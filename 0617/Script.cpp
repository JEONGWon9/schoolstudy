...
void loadScript(HWND hwnd)
{
	...
	// 중우하단 - 멘트
	setWndEditBoxLength(300);
	hEbSayComment = createWndEditBoxMultiline(100, 350, 430, 250, "Comment", NULL, ebSayRewardGoldUpdate);
...
#if 1
		if (say->nomIndex < arrayNom->count &&
			say->itemIndex < arrayItem->count &&
			say->questIndex < arrayQuest->count)
		{
			...
			btnSaySubmitUpdate(NULL);//arraySay->addObject(say);
			free(say->comment);
			free(say);
		}
		else
