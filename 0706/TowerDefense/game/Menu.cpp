...
void drawPopSlotBefore(iPopup* me, iPoint p, float dt);
...
void createPopSlot()
{
	...
	const char* strSlot[5] = {
		//"사다리", "점프", "2048", "타워디펜스"
		"The Sadari Game", "점프", "2048", "타워디펜스", "테트리스"
	};
	imgSlotBtn = (iImage**)malloc(sizeof(iImage*) * 6);
	for (int i = 0; i < 6; i++)
	{
		...
		if (i < 5)
		{
			...
			for (int j = 0; j < 2; j++)
			{
				...
				setStringBorder(0);
				float w = sizeOfString(strSlot[i]).width;
				if (w > 180)
					setStringSize(stringSize * 180 / w);
...
bool keyPopSlot(iKeyState stat, iPoint point)
{
	...
	case iKeyStateBegan:
		...
		{
			printf("selected = %d\n", i);
			METHOD_LOADING m[5] = { loadSadari, loadJump, load2048, loadTd, loadTetris };
...
void drawPopSettingsBefore(iPopup* me, iPoint p, float dt);
void createPopSettings()
{
	...
	iPoint posBtn[4] = { {60, 30}, {60, 30 + 70}, // thumb
						...
						imgSlotBtn[5]->position + iPointMake(20, 0) };
	...
	for (int i = 0; i < 4; i++)
	{
		...
		else			imgBtn = imgSlotBtn[5]->copy();		// x
		imgBtn->position = posBtn[i];
		pop->addObject(imgBtn);
