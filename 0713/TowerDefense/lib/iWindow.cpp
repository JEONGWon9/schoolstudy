...
WndCtrlSystem::WndCtrlSystem(HWND hwnd)
{
	...
	wc = (WndCtrl*)malloc(sizeof(WndCtrl) * 1000);
	wcNum = 0;
}
...
void WndCtrlSystem::update(WPARAM wParam, LPARAM lParam)
{
	...
	case WndStyle_radio:
		if( c->update )
			c->update(c->hwnd);// (HWND)lParam == c->hwnd
		break;
...
void setWndCtrlStyem(WndCtrlSystem* _wcs)
{
	wcs = _wcs;
}
	...
	const char* str, WndEditBoxStyle style, WndCtrlColor color, WndCtrlUpdate update)
{
	...
	LRESULT(CALLBACK * m[3])(HWND, UINT, WPARAM, LPARAM) = {
		WndEditBoxAllProc,WndEditBoxIntProc,WndEditBoxFloatProc
	};
	SetWindowLongPtr(hwnd, GWLP_WNDPROC, (LONG_PTR)m[style]);
	return hwnd;
}

LRESULT CALLBACK WndEditBoxMultiProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	WNDPROC wpOld = (WNDPROC)GetWindowLongPtr(hwnd, GWLP_USERDATA);
...
iOpenGL::iOpenGL(int x, int y, int width, int height, int bits, bool visible)
{
	...
	int attr[] =
	{
		WGL_CONTEXT_MAJOR_VERSION_ARB, 1,//3
		WGL_CONTEXT_MINOR_VERSION_ARB, 2,
		WGL_CONTEXT_FLAGS_ARB, 0,
		0,
	};
